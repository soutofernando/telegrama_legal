-- Promoções de produto (preço promocional e combo)

ALTER TABLE products
  ADD COLUMN preco_promocional NUMERIC(10, 2)
    CHECK (preco_promocional IS NULL OR preco_promocional >= 0),
  ADD COLUMN promo_combo_quantidade INT
    CHECK (promo_combo_quantidade IS NULL OR promo_combo_quantidade > 1),
  ADD COLUMN promo_combo_preco NUMERIC(10, 2)
    CHECK (promo_combo_preco IS NULL OR promo_combo_preco >= 0);

ALTER TABLE order_items
  ADD COLUMN valor_linha NUMERIC(10, 2)
...
CREATE OR REPLACE FUNCTION product_line_total(
  p_qty INT,
  p_preco NUMERIC,
  p_preco_promocional NUMERIC,
  p_combo_qty INT,
  p_combo_preco NUMERIC
)
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  ...
  v_remaining := p_qty;
  v_unit := COALESCE(p_preco_promocional, p_preco);

  IF p_combo_qty IS NOT NULL AND p_combo_qty > 1 AND p_combo_preco IS NOT NULL THEN
    v_total := v_total + (v_remaining / p_combo_qty) * p_combo_preco;
    v_remaining := v_remaining % p_combo_qty;
  END IF;

  v_total := v_total + v_remaining * v_unit;
