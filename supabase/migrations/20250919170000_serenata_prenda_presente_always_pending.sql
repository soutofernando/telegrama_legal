-- Serenatas, prendas and gift lines must stay pending until marked on the itinerary,
-- regardless of payment method (PIX, pay on delivery, vendor counter sale).
...
WHERE oi.product_id = p.id
  ...
  AND (
    p.tipo IN ('serenata', 'prenda')
    OR oi.eh_presente = true
  );
...
BEGIN
  ...
  LOOP
    ...
    ELSIF p_forma_pagamento = 'vendor_paid' THEN
