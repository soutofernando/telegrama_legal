-- Regular catalog lines paid on delivery are handed over at checkout (delivered).
-- Serenatas, prendas and gift lines stay pending until marked on the itinerary.
...
WHERE oi.order_id = o.id
  ...
  AND p.tipo NOT IN ('serenata', 'prenda')
  AND oi.eh_presente = false
...
BEGIN
  ...
  LOOP
    ...
    IF v_product_tipo IN ('serenata', 'prenda') OR v_eh_presente THEN
      v_line_status := 'pending'::delivery_status;
    ELSIF p_forma_pagamento IN ('vendor_paid', 'on_delivery') THEN
      v_line_status := 'delivered'::delivery_status;
    ELSE
      v_line_status := 'pending'::delivery_status;
    END IF;
